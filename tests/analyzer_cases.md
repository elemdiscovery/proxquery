...
A `&` run superimposes its compound + parts + concatenation onto one slot (see the tokenizer corpus), so the literal `'R&D'` finds the unit precisely (the `r&d` compound), the concatenation `rd` finds it, and the bare parts `r`/`d` find it. A bare `R&D` _query_ is the boolean `r & d` (the DSL reads `&` as AND) — looser, so it also matches a doc with `r` and `d` apart, where the precise literal `'R&D'` correctly misses.

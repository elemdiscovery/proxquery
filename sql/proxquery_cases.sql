-- proxquery — shared test corpus (single source of truth)
-- =======================================================
-- Each row is (label, expr, expected): `expr` is a portable SQL expression using
-- only the `ts_prox_*` surface and the two-clause proximity form, and `expected`
-- is its `::text` result (or the literal `ERR` when the expression must raise).
--
-- This file ONLY populates the `_prox_cases` temp table — it sets no search_path
-- and defines no runner, so it can be loaded ahead of any runner:
...
-- Add a case here and every runner picks it up, against both implementations.
...
-- --- cases added alongside the Rust suite (kept in lock-step) --------------
-- Distance is the absolute position gap: in `a b c`, a↔c is 2 (not 1), either order.
INSERT INTO _prox_cases(label, expr, expected) VALUES
  ('dist1',  $e$ ts_prox_match(to_tsvector('simple','a b c'),'a <~2> c') $e$, $x$true$x$),
  ('dist2',  $e$ ts_prox_match(to_tsvector('simple','a b c'),'c <~2> a') $e$, $x$true$x$),
  ('dist3',  $e$ ts_prox_match(to_tsvector('simple','a b c'),'a <~1> c') $e$, $x$false$x$),
  ('dist4',  $e$ ts_prox_match(to_tsvector('simple','a b c'),'c <~1> a') $e$, $x$false$x$),
  -- not-within where the comparison term is also the tail of the phrase operand.
  ('shp1',   $e$ ts_prox_match(to_tsvector('simple','privileged and confidential'),'confidential <!~5> "privileged and confidential"') $e$, $x$false$x$),
  ('shp2',   $e$ ts_prox_match(to_tsvector('simple','privileged and confidential w w w w w w w w confidential'),'confidential <!~5> "privileged and confidential"') $e$, $x$true$x$),
  ('shp3',   $e$ ts_prox_match(to_tsvector('simple','privileged and confidential foo confidential'),'confidential <!~5> "privileged and confidential"') $e$, $x$false$x$),
  -- native `appl:*` prefix == the `appl*` sugar, and a prefix is inclusive of the exact term.
  ('cs1',    $e$ ts_prox_query_skeleton('appl:*') $e$, $x$'appl':*$x$),
  ('cs2',    $e$ ts_prox_match(to_tsvector('simple','the apple pie'),'appl:*') $e$, $x$true$x$),
  ('cs3',    $e$ to_tsvector('simple','the apple pie') @@ ts_prox_query('appl:*') $e$, $x$true$x$),
  ('cs4',    $e$ ts_prox_match(to_tsvector('simple','apple pie'),'appl:* <~2> pie') $e$, $x$true$x$),
  ('cs5',    $e$ ts_prox_match(to_tsvector('simple','apple'),'apple*') $e$, $x$true$x$),
  ('cs6',    $e$ ts_prox_match(to_tsvector('simple','applesauce'),'apple*') $e$, $x$true$x$),
  ('cs7',    $e$ ts_prox_match(to_tsvector('simple','appl'),'apple*') $e$, $x$false$x$),
  ('cs8',    $e$ to_tsvector('simple','apple') @@ ts_prox_query('apple:*') $e$, $x$true$x$),
  -- a backslash in a literal term is harmless: no backslash lexeme ever exists.
  ('bs1',    $e$ ts_prox_match(to_tsvector('simple','a b'),'''a\b''') $e$, $x$false$x$),
  ('bs2',    $e$ ts_prox_match(to_tsvector('simple','ab'),'''a\b''') $e$, $x$false$x$),
  -- regex is anchored to the whole lexeme; user anchors / alternation stay scoped.
  ('ra1',    $e$ ts_prox_match(to_tsvector('simple','the colourful flag'),'##colou?r##') $e$, $x$false$x$),
  ('ra2',    $e$ ts_prox_match(to_tsvector('simple','the colour is nice'),'##^colou?r$##') $e$, $x$true$x$),
  ('ra3',    $e$ ts_prox_match(to_tsvector('simple','the cat sat'),'##cat|dog##') $e$, $x$true$x$),
  ('ra4',    $e$ ts_prox_match(to_tsvector('simple','category dogma'),'##cat|dog##') $e$, $x$false$x$),
  -- a hanging bare `*` is rejected, not a match-everything.
